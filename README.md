# Court Listener API PostgreSQL Extension

> [!NOTE]
>
> The Court Listener API PostgreSQL Extension is currently **experimental** and we're excited for you to experiment with it!
>
...
The REST API documentation can be found on [www.courtlistener.com](https://www.courtlistener.com/contact/).
...
CREATE EXTENSION court_listener_sdk;
```
...
pip install git+ssh://git@github.com/stainless-sdks/court-listener-sdk-python.git
```
...
- The court_listener_sdk Python package
...
```sql
ALTER DATABASE my_database SET court_listener_sdk.api_key = 'My API Key';
```

> [!NOTE]
>
> `ALTER DATABASE` persistently alters the database, but doesn't take effect until the next session. To
> ephemerally modify the current session, use `SET court_listener_sdk.api_key TO 'My API Key';`.

See this table for the available configuration parameters:

| Parameter                     | Required | Default value                                 |
| ----------------------------- | -------- | --------------------------------------------- |
| `court_listener_sdk.api_key`  | false    | -                                             |
| `court_listener_sdk.username` | false    | -                                             |
| `court_listener_sdk.password` | false    | -                                             |
| `court_listener_sdk.base_url` | false    | `'https://www.courtlistener.com/api/rest/v4'` |

## Requests and responses

To send a request to the Court Listener API, call the relevant SQL function with values corresponding to the parameter types and `SELECT` the columns you need from the returned rows.

## Pagination

For Court Listener API endpoints that return a paginated lists of results, the extension automatically fetches more pages as needed.
